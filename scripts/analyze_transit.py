"""Transit-window analysis of a real TESS SPOC light curve for WASP-69 b.
...
DATA_FILE = DATA_DIR / "tess2022217014003-s0055-0000000248853232-0242-s_lc.fits"
PLANET = "WASP-69 b"
PERIOD_DAYS = 3.86813881
EPOCH_BJD = 2455748.83428
DURATION_HOURS = 2.1869
SECTOR = 55
FIGURE_FILE = FIG_DIR / "wasp69b_tess_transit.png"
...
def main() -> dict[str, float]:
    ...
    ax.scatter(hours, (result["flux"] - 1) * 1e6, s=5, alpha=0.12, color="#5f3dc4", label="quality-filtered cadences")
    ...
    ax.plot(hours[order], (result["model"][order] - 1) * 1e6,
            color="#5f3dc4", linewidth=2.2, label="fitted box model")
    ax.axvspan(-DURATION_HOURS / 2.0, DURATION_HOURS / 2.0, color="#5f3dc4", alpha=0.08)
