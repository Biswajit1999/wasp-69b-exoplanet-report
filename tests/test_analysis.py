...
def test_pipeline_reproduces_committed_statistics():
    analysis.main()
    values = read_summary()
    assert np.isclose(values["depth_ppm"], 12462.5868391, rtol=0, atol=0.1)
    assert np.isclose(values["chi_square_flat"], 134039.416774, rtol=1e-9)
    assert np.isclose(values["chi_square_box"], 29790.1995995, rtol=1e-9)
    assert np.isclose(values["delta_chi_square"], 104249.217175, rtol=1e-9)
